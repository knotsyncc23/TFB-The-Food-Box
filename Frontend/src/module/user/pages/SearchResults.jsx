          ...
          >
            {categories.map((cat) => {
              ...
              return (
                ...
                >
                  {cat.image ? (
                    <div className={`w-16 h-16 rounded-2xl overflow-hidden border-2 transition-all ${isSelected ? 'border-red-600 dark:border-red-500 shadow-lg' : 'border-transparent'
                      }`}>
                      <img
                        src={cat.image}
                        alt={cat.name}
                    ...
                    <div className={`w-16 h-16 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center border-2 transition-all ${isSelected ? 'border-red-600 dark:border-red-500 shadow-lg bg-red-50 dark:bg-red-900/20' : 'border-transparent'
                      }`}>
                      <span className="text-xl">🍽️</span>
                    </div>
                  )}
                  <span className={`text-xs font-medium whitespace-nowrap ${isSelected ? 'text-red-700 dark:text-red-400' : 'text-gray-600 dark:text-gray-400'
                    }`}>
                    {cat.name}
                  </span>
                </button>
              ...
              return (
                ...
                >
                  {filter.hasIcon && filter.id === 'price-match' && (
                    <span className={`text-xs ${isActive ? 'text-white' : 'text-red-600 dark:text-red-400'}`}>✓</span>
                  )}
