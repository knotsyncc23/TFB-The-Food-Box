...
const getServiceAccountFromEnv = () => {
    ...
    const clientEmail = sanitizeString(config.firebaseClientEmail);
    const privateKey = normalizePrivateKey(config.firebasePrivateKey);
    const projectId = sanitizeString(config.firebaseProjectId);
    ...
    if (rawJsonParseError) {
        logger.error('Error parsing FIREBASE_SERVICE_ACCOUNT JSON:', rawJsonParseError.message);
    }
